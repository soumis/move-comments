...
include_once('moco_db.php');
include_once('moco_common.php');
...
// 		$html .= $this->debug_section();
	...
	{		
		...
		$html .= '<form name="move_comments" method="post" action="'.$_SERVER['PHP_SELF'].'?page='.$_REQUEST['page'].'&source_post_id='.$_GET['source_post_id'].'">';	
...
//		$posts = $this->db->get_all_posts();
		...
		if(!empty($posts))
		{
			...
			$html .= "<select name=\"source_post_id\" onchange=\"javascript:location.href='?page=move_comments/move_comments.php&source_post_id='+this.options[this.selectedIndex].value;\">";
